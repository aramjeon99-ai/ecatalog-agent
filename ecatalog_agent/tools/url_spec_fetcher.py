...
def _gpt_extract_specs_from_text(
    *,
    ...
    prompt = "\n".join([
        ...
        "",
        "【단위 변환 규칙】 기대 사양의 압력(Pressure Rating 등) 값은 kgf/cm² 기준입니다.",
        "  문서에서 읽은 압력이 MPa·bar·psi 등 다른 단위라면 kgf/cm²로 환산하여 비교하세요.",
        "  환산 기준: 1 kgf/cm² ≈ 0.0981 MPa, 1 MPa ≈ 10.197 kgf/cm², 1 bar ≈ 1.020 kgf/cm²",
        "",
        "출력 스키마 (JSON 한 개만):",
    ...
    *,
    ...
    content: list[dict] = [
        {"type": "text", "text": "\n".join([
            ...
            "",
            "【단위 변환 규칙】 기대 사양의 압력(Pressure Rating 등) 값은 kgf/cm² 기준입니다.",
            "  이미지에서 읽은 압력이 MPa·bar·psi 등 다른 단위라면 kgf/cm²로 환산하여 비교하세요.",
            "  환산 기준: 1 kgf/cm² ≈ 0.0981 MPa, 1 MPa ≈ 10.197 kgf/cm², 1 bar ≈ 1.020 kgf/cm²",
            "",
            "출력 스키마 (JSON 한 개만):",
            '{"model_found": true|false,',
